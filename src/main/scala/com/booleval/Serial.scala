...
object Serial {
  ...
  private implicit lazy val serializer: Writes[BooleanExpression] = {
    ...
    case Variable(symbol) => Json.obj("type" -> "var", "key" -> symbol)
  ...
  private def biOpReadsFactory[T]: BiOperatorFactory[T] => Reads[T] = factory => {
    json: JsValue =>
  ...
  private implicit lazy val readsAnd: Reads[And] = biOpReadsFactory((lhs, rhs) => And(lhs, rhs))

  private implicit lazy val readsOr: Reads[Or] = biOpReadsFactory((lhs, rhs) => Or(lhs, rhs))
  ...
  private implicit val readsVariable: Reads[Variable] = json => (JsPath \ "key").read[String].reads(json) match {
